...
class UnionBuilder(FieldBuilderBase):
    ...
    def parse(self, root):
        """
            Union instance is constructed from the <subfield> tag that is a child
        of <field>. Union's instance name is the <name> of the <subfield> <field>.
        Each struct's entry is a <name> of the <subfield>.
        Entry's bit is <subfield> "num_bits" attribute
    ...
    def _create_entry(self, field):
        ...
        for xml_subfield in xml_subfields:
            ...
            # reserved subfield names are not always unique
            if subfield_name in ['reserved', 'rsvdp', 'rsvdz']:
                subfield_name += '{}'.format(union_instance.next_rv_count)
